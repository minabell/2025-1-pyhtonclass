...
print(colors[1]) #리스트 주번째 원소 출력, 0부터 시작
...
print(cities[0:7]) #0~6번째 원소
print(cities[:-1]) #0~2까지
print(cities[3:]) #
print(cities[:]) #전부 표시
print(cities[-4:]) #뒤에서부터 4번째부터 표시
print(cities[:7:2]) #처음부터 6번째  원소까지 1칸씩 건너뜀
print(cities[::3]) #처음부터 끝까지 세칸씩 건너뜀
print(cities[::-1]) #처음부터 끝까지 방향이 거꾸로
print(cities[4::-2])
...
print(john)
del john[2] #john = [soup, [lamb, chocken]]
